...
class ParaAttentionServer:
    ...
    def _create_signal_handler(self):
        ...
        def handler(signum, frame):
            logger.debug(f"signal handler called")
            self._cleanup()
            sys.exit(0)
        return handler
    ...
    def generate(self):
        ...
        try:
            ...
            if not 'output_path' in kwargs:
                return jsonify({'error': f'request must contain an output_path'}), HTTP_STATUS['BAD_REQUEST']
            

            # validate output path, attempt to create the directory if it doesn't exist
            output_path = kwargs['output_path']
            output_directory = os.path.dirname(output_path)
            if output_directory and not os.path.exists(output_directory):
                return jsonify({'error': f'output directory {output_directory} does not exist'}), HTTP_STATUS['BAD_REQUEST']
